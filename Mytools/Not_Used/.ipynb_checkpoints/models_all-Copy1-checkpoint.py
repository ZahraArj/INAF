...
class geo_s2e_nowe(Model):
    ...
    def __init__(self, 
        ...
        self.Fully_Connected_en_1 = layers.TimeDistributed(layers.Dense(e1, activation=act_geo1, name='FC_geo_en_1', kernel_regularizer=l2(1e-4)))
        self.Fully_Connected_en_2 = layers.TimeDistributed(layers.Dense(e2, activation=act_geo2, name='FC_geo_en_2', kernel_regularizer=l2(1e-4)))        
        self.Fully_Connected_en_3 = layers.TimeDistributed(layers.Dense(e3, activation=act_geo3, name='FC_geo_en_3'))
        self.drop_1 = layers.TimeDistributed(layers.Dropout(rate=dropout_rate_1, name='drop_geo_1'))
        # ______________________________________________________________________________________________________________
        self.Fully_Connected_de_2 = layers.TimeDistributed(layers.Dense(d3, activation=act_geo4, name='FC_geo_de_1'))
...
# Geo Only _Attention Weighitng
# _________________________
class geo_s2e_att(Model):

    def __init__(self, l2_reg=1e-5, dropout_rate=0.3, act_geo='relu', act_lstm='tanh', **kwargs):
        super().__init__(**kwargs)

        self.BN_geo_input = layers.TimeDistributed(layers.BatchNormalization(name='BN_geo_inp'))

        self.Fully_Connected1 = layers.TimeDistributed(layers.Dense(128, activation=act_geo, name='FC_geo_1'))
        self.Fully_Connected2 = layers.TimeDistributed(layers.Dense(256, activation=act_geo, name='FC_geo_2'))

        self.drop_1 = layers.TimeDistributed(tf.keras.layers.Dropout(rate=0.1, name='drop_geo_1'))
        # ______________________________________________________________________________________________________________
        self.Hidden_state_1 = layers.TimeDistributed(layers.Dense(512, activation=act_geo, name='HS_1'))
        self.Hidden_state_2 = layers.TimeDistributed(layers.Dense(512, activation=act_geo, name='HS_2'))

        self.drop_2 = layers.TimeDistributed(tf.keras.layers.Dropout(rate=0.1, name='drop_geo_2'))
        # ______________________________________________________________________________________________________________

        self.Fully_Connected_all_1 = layers.TimeDistributed(layers.Dense(256, activation=act_geo, name='FC_all_1'))
        self.Fully_Connected_all_2 = layers.TimeDistributed(layers.Dense(128, activation=act_geo, name='FC_all_2'))

        self.drop_3 = layers.TimeDistributed(tf.keras.layers.Dropout(rate=0.1, name='drop_geo_3'))
        
        self.AttentionLayer = layers.TimeDistributed(layers.Attention(use_scale=True))
        self.drop_att = layers.TimeDistributed(tf.keras.layers.Dropout(rate=0.1, name='drop_geo_att'))
        self.BN_att = layers.TimeDistributed(layers.BatchNormalization(name='BN_att'))
        # self.concatenate_att = tf.keras.layers.Concatenate(axis=2)
        self.concatenate_att = tf.keras.layers.Add()
        # ______________________________________________________________________________________________________________
        self.RNN_LSTM_1 = layers.LSTM(64,
                                      activation=act_lstm,
    ...
    def call(self, geo_lidar_input: tf.Tensor, training: bool = True):
        ...
        x1_geo = self.Fully_Connected1(geo_inp, training=training)
        x2_geo = self.Fully_Connected2(x1_geo, training=training)
        x2_geo = self.drop_1(x2_geo, training=training)

        # ______________________________________________________________________________________________________________
        geo_branch_1 = self.Hidden_state_1(x2_geo, training=training)
        geo_branch_2 = self.Hidden_state_2(geo_branch_1, training=training)
        geo_branch_2 = self.drop_2(geo_branch_2, training=training)
        # ______________________________________________________________________________________________________________
        combined_1 = self.Fully_Connected_all_1(geo_branch_2, training=training)
        # combined_1 = self.Fully_Connected_all_1_act(combined_1, training=training)
        combined_2 = self.Fully_Connected_all_2(combined_1, training=training)
        # combined_2 = self.Fully_Connected_all_2_act(combined_2, training=training)

        combined_2 = self.drop_3(combined_2, training=training)
        
        attention_output = self.AttentionLayer([combined_2, combined_2])
        attention_output = self.drop_att(attention_output, training=training)
        
        attention_output =self.concatenate_att([combined_2, attention_output])
        attention_block_output = self.BN_att(attention_output)
        # ______________________________________________________________________________________________________________
        # outp = self.RNN_LSTM(combined_2, training=training)
        outp = self.RNN_LSTM_1(attention_block_output, training=training)
        outp = self.RNN_LSTM_2(outp, training=training)
        outp = self.Fully_Connected_end(outp)
...
class geo_s2e_att_two(Model):
    ...
    def __init__(self, l2_reg=1e-5, act_geo='relu', act_lstm='tanh', **kwargs):
        super().__init__(**kwargs)
...
class LiD_s2e_att(Model):
    ...
    def __init__(self, act_geo='relu', act_lstm='tanh',**kwargs):
        super().__init__(**kwargs)
        
        self.create_res_net = layers.TimeDistributed(create_res_net_NT_one(training=True))   
        
        # self.Fully_Connected_li0 = layers.TimeDistributed(layers.Dense(1024, activation=act_li, name='FC_lidar_0')) #1
        self.Fully_Connected_li1 = layers.TimeDistributed(layers.Dense(512, activation=act_li, name='FC_lidar_1')) #1
        self.Fully_Connected_li2 = layers.TimeDistributed(layers.Dense(256, activation=act_li, name='FC_lidar_2')) #2
        self.Fully_Connected_li3 = layers.TimeDistributed(layers.Dense(128, activation=act_li, name='FC_lidar_3')) #3
        ...
        self.Dropout_li1 = layers.TimeDistributed(layers.Dropout(0.2, name='dropout_lidar_1'))
        self.Dropout_li2 = layers.TimeDistributed(layers.Dropout(0.2, name='dropout_lidar_2'))
        self.Dropout_li3 = layers.TimeDistributed(layers.Dropout(0.2, name='dropout_lidar_3'))
        self.Dropout_li4 = layers.TimeDistributed(layers.Dropout(0.2, name='dropout_lidar_4'))
    ...
    def call(self, geo_lidar_input, training: bool = True):
        ...
        # x12 = self.conc12([x1_Resnet,x2_Resnet])
        
        # FC1 = self.Dropout_li0(self.Fully_Connected_li0(x12))
        FC2 = self.Dropout_li1(self.Fully_Connected_li1(x_Resnet))
        FC3 = self.Dropout_li2(self.Fully_Connected_li2(FC2))
        FC4 = self.Dropout_li3(self.Fully_Connected_li3(FC3))
        FC5 = self.Dropout_li4(self.Fully_Connected_li4(FC4))
        
        # Apply self-attention
        FC5_att = self.Attention_li(FC5, FC5, FC5)
        
        combined = self.RNN_LSTM_1(FC5_att)
        combined = self.RNN_LSTM_2(combined)
...
class Combined_s2e_nowe_simple(Model):
    ...
    def __init__(self, 
                 #_____________________________________________________geo branch
                 act_geo1='relu', act_geo2='relu', #act_geo3='relu', 
                 act_geo4='relu',                 
                 dropout_rate_1_geo=0.3, #dropout_rate_2_geo = 0.2, 
                 e1_geo=128, e2_geo=256, #e3_geo=512, 
                 d3_geo=256, #d2_geo=192,
                 act_geo_lstm1='relu', act_geo_lstm2='relu', 
                 # do_lstm_geo_1=0.2, do_lstm_geo_2=0.2, 
                 ...
                 d1_li=256, d2_li=128, #d3_li=64, d4_li=32,
                 # d1_lstm_li=32,
                 # act_li_lstm1='tanh', act_li_lstm2='tanh', 
                 act_li_lstm3='tanh', act_li_lstm4='tanh', 
                 # do_lstm_li_1=0.1, do_lstm_li_2=0.1, 
        ...
        # Define the layers for feature fusion
        self.dense_fusion = layers.Dense(, activation=act_fuse, name='fusion_dense')
        self.dropout_fusion = layers.Dropout(dropout_rate_fuse, name='fusion_dropout')
