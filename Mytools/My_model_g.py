...
# ---------------------------------------------
# TensorFlow Setup (GPU, Mixed Precision, etc.)
# ---------------------------------------------
...
from Mytools.my_tuner import build_model_geo, build_model_geo_att, build_model_lidar, build_model_lidar_att, build_model_all, build_model_all_att

# Models
from Mytools.models_all import (
    GeoLayer, GeoLayer_two, GeoLayer_intermed, GeoLayer_end,
    geo_s2e_att, geo_s2e_att_two, geo_s2e_nowe, geo_s2e_nowe_two,
    Lidar_layer, Lidar_layer_two, Lidar_layer_intermed,
    LiD_s2e_att, LiD_s2e_att_two, LiD_s2e_nowe, LiD_s2e_nowe_two,
    LiD_s2e_INAF, LiD_s2e_INAF_two,
    Combined_weighted, Combined_weighted_simple,
    Combined_s2e_nowe, Combined_s2e_nowe_two,
    Combined_s2e_nowe_simple, Combined_s2e_nowe_two_simple,
    Combined_s2e_att, Combined_s2e_att_two,
    Combined_s2e_att_simple, Combined_s2e_att_two_simple,
    Combined_s2e_soft_nowe, Combined_s2e_soft_nowe_two,
    Combined_s2e_INAF
)
...
class BaseNet:
    def __init__(self, mother_folder, manual_id=None):
        """
        Base class for training and evaluation of dual-branch (Geo & Lidar) networks.
        """
        # ---------------------------------------------------------
        # Initialization
        # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
        # Time-stamped Folder for Output Saving
        # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
        # Combined2 - First Model Selection
        # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
        # Combined2_two - Second Model Selection
        # ---------------------------------------------------------
    ...
    def makemodel(self):
        ...
        if self.branch_mode == 'geo':
            ...
            if self.fusion == 'soft':
                build_model = lambda hp: build_model_geo_att(hp, model_combined2, combined_loss, optimizer)
        elif self.branch_mode == 'lidar':
            if self.fusion == 'direct':
                build_model = lambda hp: build_model_lidar(hp, model_combined2, combined_loss, optimizer)
            if self.fusion == 'soft':
                build_model = lambda hp: build_model_lidar_att(hp, model_combined2, combined_loss, optimizer)
        elif self.branch_mode == 'all': 
            if self.fusion == 'direct':
